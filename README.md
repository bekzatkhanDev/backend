...
```bash
docker exec tax_service-backend-1 python manage.py makemigrations
```
...
### Тестовые поездки (mock data)

`seed_data` создаёт историю поездок для первых 4 клиентов:

| Аккаунт | Телефон | Поездки | Оценено | Без оценки | Отменено |
|---------|---------|---------|---------|------------|---------|
| Customer1 | `+77010000001` | 5 | 2 | **2** | 1 |
| Customer2 | `+77010000002` | 4 | 1 | **2** | 1 |
| Customer3 | `+77010000003` | 2 | 0 | **2** | 0 |
| Customer4 | `+77010000004` | 2 | 1 | **1** | 0 |

**Рекомендуется для демонстрации функции отзывов:** войти как `+77010000001` — у неё смешанная история (есть оценённые и неоценённые поездки), кнопка «Оценить поездку» будет видна для 2 завершённых поездок.

Водители `Driver1` (`+77020000001`) и `Driver2` (`+77020000002`) получили наибольшее количество отзывов и участвовали в большинстве тестовых поездок.
